...
const redis = require('../utils/redis'); // 引入 redis 工具
const {
  getProfileByName,
  verifyPassword,
  generateAccessToken,
...
// 内存存储用于serverId验证（保留用作 join 缓存，hasJoined 现在主要走 Redis/数据库）
...
router.post('/sessionserver/session/minecraft/join', async (req, res) => {
  try {
    ...
    // 内存缓存保留（可选，即使使用Redis也可以保留本地快速校验）
    ...
    // 同时写入 Redis，供 hasJoined 兜底使用
    const redisKey = `ygg_join:${serverId}:${tokenRecord.player_name}`;
    try {
      await redis.set(redisKey, JSON.stringify({
        ...
        ip: req.ip,
        timestamp: Date.now()
      }), 'EX', 30); // 30 秒过期，与 Minecraft 服务端验证窗口匹配
    } catch (redisErr) {
      routeLog('warn', 'Redis 写入失败（非致命）', { error: redisErr.message });
    }
...
router.get('/sessionserver/session/minecraft/hasJoined', async (req, res) => {
  try {
    ...
    const redisKey = `ygg_join:${serverId}:${username}`;
    ...
    try {
      ...
      routeLog('warn', 'Redis 读取失败，使用数据库兜底', { error: err.message });
    }

    if (cachedData) {
      const cacheEntry = JSON.parse(cachedData);
      routeLog('debug', 'hasJoined: 命中 Redis 缓存', { username, serverId });

      // IP 校验（如果服务端提供）
      if (ip && cacheEntry.ip && cacheEntry.ip !== ip) {
        routeLog('warn', 'hasJoined: IP 不匹配（缓存）', { cachedIp: cacheEntry.ip, requestIp: ip });
        return res.status(204).end();
      }
      ...
      if (!profile || profile.is_banned) {
        routeLog('warn', 'hasJoined: 缓存对应的角色不存在或已封禁');
        return res.status(204).end();
      }

      // 构造标准响应
      const uuid = formatUuid(profile.uuid).replace(/-/g, '');
      ...
      if (profile.cape_url) {
        texturePayload.textures.CAPE = { url: getProxyUrl(profile.cape_url, req) };
      }
      ...
      const response = {
        ...
        properties: [
          { name: 'textures', value: base64Value, signature: signature || '' },
          { name: 'uploadableTextures', value: 'skin,cape' }
        ]
      };

      routeLog('info', 'hasJoined 验证成功 (Redis缓存)', { username });
      console.log(`[Yggdrasil] 验证成功: ${username}${signature ? ' (已签名)' : ''}`);
      return res.json(response);
    }

    // 2. 缓存不存在或已过期 -> 数据库兜底查询
    routeLog('debug', 'hasJoined: 缓存未命中，使用数据库兜底查询', { username });

    const profile = await getProfileByName(username);
    if (!profile || profile.is_banned) {
      routeLog('warn', 'hasJoined: 数据库兜底查询失败或角色封禁', { username });
      return res.status(204).end();
    }
    ...
    // 写入 Redis 缓存，过期时间 5 分钟，减轻后续数据库压力
    try {
      await redis.set(redisKey, JSON.stringify({
        uuid: profile.uuid,
        ip: req.ip,
    ...
    routeLog('info', 'hasJoined 验证成功 (数据库兜底)', { username });
    console.log(`[Yggdrasil] 验证成功: ${username}${signature ? ' (已签名)' : ''}`);
    ...
    routeLog('error', 'hasJoined 内部错误', { error: error.message });
    res.status(204).end();
  }
});
